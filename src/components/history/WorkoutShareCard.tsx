...
import { summarizeWorkoutDay, renderWorkoutCard, recommendWorkoutQuote, loadWorkoutPhoto, WORKOUT_QUOTE_CATALOG } from '../../utils/workoutCard';
...
export function WorkoutShareCard({ sessions, date, unit, onClose }: { sessions: WorkoutSession[]; date: string; unit: WeightUnit; onClose: () => void }) {
  ...
  const selectedQuote = WORKOUT_QUOTE_CATALOG.find(quote => t(quote.text) === title);
  const anatomyReady = useRef<Promise<HTMLImageElement | undefined>>();
  ...
  useEffect(() => {
    ...
    if (!anatomyReady.current) {
      const anatomy = new Image();
      anatomy.src = '/anatomy/muscle-atlas.png';
      anatomyReady.current = Promise.all([document.fonts.ready, anatomy.decode()]).then(() => anatomy).catch(() => undefined);
    }
    // Coalesce typing and slider updates; reuse the decoded atlas for this dialog.
    const timer = window.setTimeout(() => {
      anatomyReady.current!.then(anatomy => {
        if (!active) return;
        if (!anatomy) throw new Error('atlas unavailable');
        try { setImage(renderWorkoutCard(summary, { light, title, author: selectedQuote?.author ? t(selectedQuote.author) : undefined, photo, textColor, overlay, anatomy })); }
        catch { setImage(''); setMessage(t('이미지 생성에 실패했습니다. 다시 열어 주세요.')); }
        finally { setPreviewBusy(false); }
      }).catch(() => { if (active) { setPreviewBusy(false); setImage(''); anatomyReady.current = undefined; setMessage(t('해부도 이미지를 불러오지 못했습니다. 다시 열어 주세요.')); } });
    }, 150);
    return () => { active = false; window.clearTimeout(timer); };
  }, [summary, light, title, photo, textColor, overlay, selectedQuote]);
  return <div className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-3" onClick={onClose}>
    <div ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="workout-card-title" className="bg-white dark:bg-[#1C1C1E] rounded-3xl w-full max-w-lg max-h-[94dvh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()} onKeyDown={e => {
      ...
      if (e.key === 'Tab') {
        const buttons = [...(dialog.current?.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), a[href]') || [])].filter(node => node.getClientRects().length > 0);
      ...
      <div className="overflow-y-auto min-h-0 px-4 pb-4 space-y-4">
        <div className="rounded-2xl bg-[#F2F2F7] dark:bg-[#2C2C2E] p-3 space-y-3">
          ...
          {selectedQuote?.author && <p className="text-xs text-gray-500">— {t(selectedQuote.author)} · <a className="underline" href={selectedQuote.source} target="_blank" rel="noopener noreferrer">{t("출처 보기")}</a></p>}
          <div>
