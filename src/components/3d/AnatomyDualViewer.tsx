    ...
    <section aria-label="전면·후면 근육 해부도" className="overflow-hidden rounded-2xl bg-[#101014] text-white border border-white/10">
      ...
      <div className="px-3 pb-3 flex items-center justify-between gap-2">
        <div className="flex gap-1 rounded-lg bg-white/5 p-1">
          {(['both','front','back'] as const).map(mode => <button key={mode} type="button" aria-pressed={view === mode} onClick={() => setView(mode)} className={`px-3 py-1.5 rounded-md text-xs font-semibold ${view === mode ? 'bg-white/15 text-white' : 'text-gray-400'}`}>{mode === 'both' ? t("전체") : mode === 'front' ? '전면' : '후면'}</button>)}
        </div>
