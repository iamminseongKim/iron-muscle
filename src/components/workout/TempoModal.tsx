      ...
      <div className="bg-white dark:bg-[#1C1C1E] border border-black/10 dark:border-white/10 rounded-3xl w-full max-w-sm overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="p-4 border-b border-black/5 dark:border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-extrabold text-base text-[#1D1D1F] dark:text-white">{t("수축·이완 템포 (TUT)")}</h3>
              <p className="text-xs text-gray-400">{t("근육 긴장 지속시간을 통한 점진적 과부하")}</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2.5 bg-[#F2F2F7] dark:bg-[#2C2C2E] rounded-2xl border border-black/5 dark:border-white/5">
              <span className="text-[11px] text-gray-500 dark:text-gray-400 font-bold block mb-1.5 whitespace-nowrap">{t("이완 (내릴때)")}</span>
            ...
            <div className="p-2.5 bg-[#F2F2F7] dark:bg-[#2C2C2E] rounded-2xl border border-black/5 dark:border-white/5">
              <span className="text-[11px] text-gray-500 dark:text-gray-400 font-bold block mb-1.5 whitespace-nowrap">{t("정지 (바닥)")}</span>
              <div className="flex items-center justify-center gap-1">
                <button
                  type="button"
            ...
            <div className="p-2.5 bg-[#F2F2F7] dark:bg-[#2C2C2E] rounded-2xl border border-black/5 dark:border-white/5">
              <span className="text-[11px] text-gray-500 dark:text-gray-400 font-bold block mb-1.5 whitespace-nowrap">{t("수축 (밀때)")}</span>
              <div className="flex items-center justify-center gap-1">
                <button
                  type="button"
          ...
          <div className="flex items-center justify-between px-3 py-2 bg-[#F2F2F7] dark:bg-[#2C2C2E] rounded-2xl text-xs font-semibold">
            <span className="text-gray-500 dark:text-gray-400">{t("1회 반복당 긴장 시간:")}</span>
            <span className="font-extrabold text-[#1D1D1F] dark:text-white text-sm">
              {t("총")} <span className="text-[#FF9500] font-black">{totalSecPerRep}{t("초")}</span> ({t("10회 시")} {totalSecPerRep * 10}{t("초 지속")})
            </span>
          </div>

          {/* 프리셋 리스트 */}
          <div className="space-y-1.5">
            <span className="text-xs font-bold text-gray-400 block mb-1">{t("추천 템포 프리셋")}</span>
            {presets.map((p, idx) => (
              <button
                key={idx}
                type="button"
              ...
              >
                <div>
                  ...
                  <span className="text-[11px] text-gray-400">{t(p.sub)}</span>
                </div>
          ...
          <div className="flex items-start gap-2 p-2.5 bg-blue-500/10 dark:bg-blue-950/30 rounded-2xl border border-blue-500/20 text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed">
            ...
            <span>{t("무게가 늘지 않더라도, 이완(네거티브) 속도를 1초 늘리면 근육에 실질적인 과부하가 발생합니다!")}</span>
          </div>
        </div>
